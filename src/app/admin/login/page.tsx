...
const LoginPage = () => {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (result.success && result.user) {
      if (result.user.role?.toUpperCase() !== "ADMIN") {
        await logout();
        setErrorMessage("คุณไม่มีสิทธิ์เข้าใช้งานส่วนผู้ดูแลระบบ (สำหรับ Admin เท่านั้น)");
        return;
      }
  ...
  return (
    <div className="mx-auto max-w-md w-full px-4 py-12">
      ...
      {errorMessage && (
        <div className="mb-5 rounded-md bg-red-50 p-3.5 text-sm text-red-600 border border-red-200">
          {errorMessage}
        </div>
      )}
